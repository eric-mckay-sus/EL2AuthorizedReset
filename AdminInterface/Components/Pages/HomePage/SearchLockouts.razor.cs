...
public partial class SearchLockouts
{
    ...
    private async Task HandleClear() => await ClearAllFilters();
    ...
    protected override IQueryable<LockoutReset> ApplyFilters(IQueryable<LockoutReset> query)
    {
        ...
        if (FilterReason.IsActive)
            query = query.Where(x => x.Reason != null && x.Reason.Contains(FilterReason.Value!));
