...
/// <param name="cache">Cache used to store the resolved ClaimsPrincipal so the DB isn't hit every time navigation occurs.</param>
public class UserIdentityService(IDbContextFactory<AuthResetDbContext> dbFactory, IMemoryCache cache) : IUserIdentityService
{
    private readonly IDbContextFactory<AuthResetDbContext> dbFactory = dbFactory; // shadowing the parameter name with the field name is okay because we always qualify with 'this'
